...
const listAllClosedTradesByUser = `-- name: ListAllClosedTradesByUser :many
SELECT
    guid,
    user_id,
    inst_id,
...
const listTradeFills = `-- name: ListTradeFills :many
SELECT
    trade_guid,
    trade_id,
    order_id,
...
type ListTradeFillsRow struct {
	TradeGuid   uuid.UUID          `db:"trade_guid"`
	...
	FillSize    float64            `db:"fill_size"`
	Fee         float64            `db:"fee"`
	RealizedPnl float64            `db:"realized_pnl"`
	FilledAt    pgtype.Timestamptz `db:"filled_at"`
}

func (q *Queries) ListTradeFills(ctx context.Context, db DBTX, tradeGuid uuid.UUID) ([]*ListTradeFillsRow, error) {
	rows, err := db.Query(ctx, listTradeFills, tradeGuid)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []*ListTradeFillsRow
	for rows.Next() {
		var i ListTradeFillsRow
		if err := rows.Scan(
			&i.TradeGuid,
			&i.TradeID,
			&i.OrderID,
...
const upsertTradeFill = `-- name: UpsertTradeFill :exec
...
type UpsertTradeFillParams struct {
	TradeGuid   uuid.UUID          `db:"trade_guid"`
	TradeID     string             `db:"trade_id"`
	OrderID     string             `db:"order_id"`
	AlgoID      string             `db:"algo_id"`
...
func (q *Queries) UpsertTradeFill(ctx context.Context, db DBTX, arg *UpsertTradeFillParams) error {
	_, err := db.Exec(ctx, upsertTradeFill,
		arg.TradeGuid,
		arg.TradeID,
		arg.OrderID,
		arg.AlgoID,
